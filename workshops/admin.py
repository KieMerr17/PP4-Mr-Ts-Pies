...
class BookingAdmin(admin.ModelAdmin):
    list_display = (
        'user', 'name', 'phone_number', 'workshop', 'spaces',
        'dietary_requirements', 'booked_on', 'approved'
        )
        ...
        for booking in queryset:
            if not booking.approved:
                ...
                if new_spaces < 0:
                    messages.error(
                        request,
                        f"Spaces for this booking exceed the spaces available."
                        )
                else:
